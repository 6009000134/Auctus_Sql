...
BEGIN
 ...
 --   BEGIN
	...
	--判断SN码是否已经在本单扫描过
	IF EXISTS(SELECT 1 FROM dbo.TP_RDRcvDetail a WHERE a.RcvID=@RcvID AND a.SNCode=ISNULL(@SNCode,''))
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.TP_RDRcvDetail a WHERE a.RcvID<>@RcvID AND a.SNCode=ISNULL(@SNCode,''))
	BEGIN
		SELECT '0'MsgType,'['+@SNCode+']已经在'+(SELECT a.DocNo FROM dbo.TP_RDRcv a WHERE a.ID=@RcvID)+'单内扫描过！'Msg	
		RETURN;
	END 
	...
	BEGIN
		...
		SELECT @CreateBy,GETDATE(),@RcvID,@SNCode,b.MaterialID,c.MaterialCode,c.MaterialName,@Status,@Progress,@Remark FROM dbo.opPlanExecutMain a INNER JOIN dbo.mxqh_plAssemblyPlanDetail b ON a.AssemblyPlanDetailID=b.ID

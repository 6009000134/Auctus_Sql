...
商务中心入库记录扫码
*/
ALTER PROC [dbo].[sp_TP_BCRcvScan]
(
@pageIndex INT,
@pageSize INT,
@SNCode VARCHAR(100),
...
BEGIN
 ...
 --   BEGIN
	...
	--判断SN码是否已经在本单扫描过
	IF EXISTS(SELECT 1 FROM dbo.TP_BCRcvDetail a WHERE a.RcvID=@RcvID AND a.SNCode=ISNULL(@SNCode,''))
	BEGIN
	...
	IF EXISTS(SELECT 1 FROM dbo.TP_BCRcvDetail a WHERE a.RcvID<>@RcvID AND a.SNCode=ISNULL(@SNCode,''))
	BEGIN
		SELECT '0'MsgType,'['+@SNCode+']已经在'+(SELECT a.DocNo FROM dbo.TP_BCRcv a WHERE a.ID=@RcvID)+'单内扫描过！'Msg	
		RETURN;
	END 
	--根据码查出内控码
	...
	BEGIN
		...
		SELECT @CreateBy,GETDATE(),@RcvID,@SNCode,b.MaterialID,c.MaterialCode,c.MaterialName,@TypeID,@TypeCode,@TypeName,@Remark FROM dbo.opPlanExecutMain a INNER JOIN dbo.mxqh_plAssemblyPlanDetail b ON a.AssemblyPlanDetailID=b.ID

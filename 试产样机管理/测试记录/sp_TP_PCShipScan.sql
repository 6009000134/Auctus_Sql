...
BEGIN
 ...
 --   BEGIN
	...
	--判断SN码是否已经在本单扫描过
	IF EXISTS(SELECT 1 FROM dbo.TP_PCShipDetail a WHERE a.ShipID=@ShipID AND a.SNCode=ISNULL(@SNCode,''))
	BEGIN
	...
	BEGIN
		...
		SELECT @CreateBy,GETDATE(),@ShipID,@SNCode,b.MaterialID,c.MaterialCode,c.MaterialName,@Status,@Progress,@SoftUpdateDate,@Remark FROM dbo.opPlanExecutMain a INNER JOIN dbo.mxqh_plAssemblyPlanDetail b ON a.AssemblyPlanDetailID=b.ID

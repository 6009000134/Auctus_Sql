...
WHERE 1=1
AND (
 a.WorkName IN ('启动时间','Alpha日期','Beta日期','转量产日期','Seedstock日期','立项评审','转产评审','量产评审')
 OR a.WorkName LIKE '功能样机%' OR a.WorkName LIKE '工程样机%'
)
),
PivotData AS
(
...
,CASE WHEN a.WorkName='Alpha日期' OR a.WorkName LIKE '功能样机%' THEN a.PlanStartDate END '功能样机' 
,CASE WHEN a.WorkName='Beta日期' OR a.WorkName LIKE '工程样机%' THEN a.PlanStartDate END '工程样机' 
...
,CASE WHEN a.WorkName='Alpha日期' OR a.WorkName LIKE '功能样机%' THEN a.ActualStartDate END 'Alpha日期2' 
,CASE WHEN a.WorkName='Beta日期' OR a.WorkName LIKE '工程样机%' THEN a.ActualStartDate END 'Beta日期2' 
...
,CASE WHEN a.WorkName='启动时间' OR a.WorkName='立项评审' THEN '立项评审' 
WHEN a.WorkName='Alpha日期' OR a.WorkName LIKE '功能样机%' THEN   '功能样机' 
WHEN a.WorkName='Beta日期' OR a.WorkName LIKE '工程样机%' THEN   '工程样机' 
WHEN a.WorkName='转量产日期' OR a.WorkName='转产评审' THEN   '转量产评审' 
WHEN a.WorkName='Seedstock日期' OR a.WorkName='量产评审' THEN   'Seedstock' 
...
(
SELECT a.PID
,MIN(a.启动时间)启动时间,MIN(a.功能样机)Alpha日期,MIN(a.工程样机)Beta日期,MIN(a.转量产日期)转量产日期,MIN(a.Seedstock日期)Seedstock日期 
...
AND ISNULL(sd.PropertyValue,'')NOT IN ('','无') 
AND ISNULL(cus.PropertyValue,'无') NOT IN ('无','')
...
AND c.ManagerName!='蔡东志')a,#TempTable b
WHERE b.Saler='全部销售'
UNION ALL
--非全部销售
...
UNION
SELECT a.*,b.* FROM (SELECT '葛笑节' Rcv,'gexj@auctus.cn' RcvMail)a,#TempTable b
) t

...
WHERE 1=1
AND c.WorkName IN ('启动时间','Alpha日期','Beta日期','转量产日期','Seedstock日期')
),
PivotData AS
(
SELECT a.PID,a.Principal,a.ProjectCode,a.ProjectName,a.Priority,a.CategoryId
,CASE WHEN a.WorkName='启动时间' THEN a.PlanStartDate END '启动时间' 
,CASE WHEN a.WorkName='Alpha日期' THEN a.PlanStartDate END 'Alpha日期' 
,CASE WHEN a.WorkName='Beta日期' THEN a.PlanStartDate END 'Beta日期' 
,CASE WHEN a.WorkName='转量产日期' THEN a.PlanStartDate END '转量产日期' 
,CASE WHEN a.WorkName='Seedstock日期' THEN a.PlanStartDate END 'Seedstock日期' 
FROM ProjectInfo a
),
WorkPieces AS
(
